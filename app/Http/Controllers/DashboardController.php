...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $todayPresence = DB::table('absensis')
            ->where([['tgl_absensi', date('Y-m-d')], ['nik', Auth::guard('pegawai')->user()->nik]])
            ->first();
        ...
        $recapData = DB::table('absensis')
            ->selectRaw('COUNT(nik) as sum_presence, SUM(IF(jam_in > "07:00",1,0)) as sum_late')
            ->where('nik', Auth::guard('pegawai')->user()->nik)
            ->whereRaw('MONTH(tgl_absensi)="' . date('m') . '"')
            ->whereRaw('YEAR(tgl_absensi)="' . date('Y') . '"')
            ->first();
        ...
        # Leaderboard
        $leaderBoard = DB::table('absensis')
            ->leftJoin('pegawais', 'pegawais.nik', '=', 'absensis.nik')
            ->where('absensis.tgl_absensi', date('Y-m-d'))
            ->get();
